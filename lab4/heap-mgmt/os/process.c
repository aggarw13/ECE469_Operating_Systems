...
int ProcessFork (VoidFunc func, uint32 param, char *name, int isUser) {
  ...
  uint32 new_page;         // Used for syoring newly allocated physical page
  ...
  pcb->pagetable[MEM_MAX_VIRTUAL_ADDRESS >> MEM_L1FIELD_FIRST_BITNUM] = MemorySetupPte(new_page);
  ...
  //printf("Allocated page for user stack!\n");
...
void main (int argc, char *argv[])
{
  ...
  dbprintf ('i', "Set timer quantum to %d, about to run first process.\n",
	    processQuantum);
  intrreturn ();
  printf("Completed Process Forking!\n");

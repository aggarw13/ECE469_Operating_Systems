...
static l2_pagetable l2_pagetables[MEM_MAX_L2_PAGETABLES];
...
void MemoryModuleInit() {
  int i;
  uint32 lastOSaddr_page = (lastosaddress >> MEM_L2FIELD_FIRST_BITNUM);
...
uint32 MemoryTranslateUserToSystem (PCB *pcb, uint32 addr) {
  ...
  //int vpage_no = addr >> MEM_L2FIELD_FIRST_BITNUM;
  
  uint32 l2_pageno = ((addr >> MEM_L2FIELD_FIRST_BITNUM) & MEM_L2_PAGENUM_MASK);
  uint32 l1_pageno = addr >> MEM_L1FIELD_FIRST_BITNUM; 
  int pt_entry = ((uint32*)pcb->pagetable[l1_pageno])[l2_pageno];
  //printf("MemoryTranslateToUser() : vaddr = 0x%x\n", addr);
...
int MemoryPageFaultHandler(PCB *pcb) {
  ...
  uint32 vpagenum = addr >> MEM_L2FIELD_FIRST_BITNUM, ppagenum;
  ...
  checkAndAllocateL2pagetable(pcb, l1_pageno);
  ((uint32*)pcb->pagetable[l1_pageno])[l2_pageno] = MemorySetupPte(ppagenum);
...
int MemoryAllocPage(void) {
  int phys_page = lastosaddress >> MEM_L2FIELD_FIRST_BITNUM;
...
uint32 MemorySetupPte (uint32 page) {
  ...
  uint32 pte = (uint32)((page << MEM_L2FIELD_FIRST_BITNUM) + MEM_PTE_VALID);
...
void MemoryFreePage(uint32 page) {
  ...
  //printf("Currently considering physical page : %d\n", page);
  //printf("Offset = %d", offset); 
 // if (offset == 0) {
 //   freemap[page >> 5] &= 0xFFFFFFFF;
 // } else {
    //printf("efore freeing physical page : 0x%x with BITWSIE AND value : 0x%x\n", page, invert((0x01 << (31 - offset))));

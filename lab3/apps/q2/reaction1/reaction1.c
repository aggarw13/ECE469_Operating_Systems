...
void main(int argc, char * argv[])
{
  ...
  char hydro[HYDRO_LEN], hydrogen_mesg[H_LEN] = "H2", oxygen_mesg[O_LEN] = "O2";
	...
	mbox_hydrogen = dstrtol(argv[2], NULL, 10);
  ...
  {
  	if (mbox_recv(mbox_hydro, HYDRO_LEN, (void *)&hydro) == MBOX_FAIL) {
  ...
  for(j = 0; j < O_NUM; j++)
   ...
   if (mbox_close(mbox_hydro) == MBOX_FAIL) {
      Printf("reaction 1 (%d): Could not close the water mailbox!\n", getpid());
      Exit();
    }

  if (mbox_close(mbox_hydrogen) == MBOX_FAIL) {
      Printf("reaction 1 (%d): Could not close the hydrogen mailbox!\n", getpid());
      Exit();
    }

  if (mbox_close(mbox_oxygen) == MBOX_FAIL) {
      Printf("reaction 1 (%d): Could not close the oxygen mailbox!\n", getpid());
      Exit();
    }
